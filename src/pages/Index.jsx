...
const Index = () => {
  ...
  const [editingGratitude, setEditingGratitude] = useState({ text: "", index: null });
          ...
          <List>
            {gratitudes[new Date().toISOString().split("T")[0]]?.map((item, index) => (
              <ListItem key={index} my={2}>
                {editingGratitude.index === index ? (
                  <HStack spacing={2}>
                    <Input value={editingGratitude.text} onChange={(e) => setEditingGratitude({ ...editingGratitude, text: e.target.value })} />
                    <IconButton
                      aria-label="Save"
                      icon={<FaSave />}
                      onClick={() => {
                        const today = new Date().toISOString().split("T")[0];
                        const updatedGratitudes = gratitudes[today].map((g, i) => (i === index ? editingGratitude.text : g));
                        setGratitudes({ ...gratitudes, [today]: updatedGratitudes });
                        setEditingGratitude({ text: "", index: null });
                      }}
                    />
                    <IconButton aria-label="Cancel" icon={<FaTimes />} onClick={() => setEditingGratitude({ text: "", index: null })} />
                  </HStack>
                ) : (
                  <HStack spacing={2}>
                    <ListIcon as={FaPlus} color="green.500" />
                    <Text onClick={() => setEditingGratitude({ text: item, index })}>{item}</Text>
                    <IconButton aria-label="Edit gratitude" icon={<FaEdit />} size="sm" onClick={() => setEditingGratitude({ text: item, index })} />
